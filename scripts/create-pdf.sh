...
if command -v pandoc &> /dev/null; then
    if [ "$CONTENT_TYPE" = "markdown" ]; then
        pandoc "$TEMP_FILE" -o "$OUTPUT_FILE" --pdf-engine=xelatex 2>/dev/null \
        || pandoc "$TEMP_FILE" -o "$OUTPUT_FILE" --pdf-engine=pdflatex 2>/dev/null \
        || pandoc "$TEMP_FILE" -o "$OUTPUT_FILE" 2>/dev/null
    else
        pandoc "$TEMP_FILE" -o "$OUTPUT_FILE" 2>/dev/null
    fi
